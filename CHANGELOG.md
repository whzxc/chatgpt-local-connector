...
- fix(codex): 适配桌面版内置程序新路径
- fix(usage): 隐藏备用模型额度池
- fix(readers): 移除外部数据读取的多余校验
...
- Windows 改为仅提供 EXE 安装包，定制中文安装界面，并优化图标和高分屏下的路径提示。
- 统一托盘详情与应用面板的交互，改进订阅额度浮层和用量历史展示。
- 首页新增 GitHub 入口，优化 Agent 展示设置、连接拓扑和窗口布局。
...
- 状态栏面板可根据内容调整高度，并与 Agent 展示设置保持同步。
- 修复 Windows 窗口激活时原生标题栏外观不同步的问题。
- 调整订阅额度详情的触发范围，修正归档任务的状态展示。
...
- 重构托盘面板，集中展示连接状态与订阅用量，并提供更完整的用量详情。
- 优化用量历史与分享展示，改进 macOS 原生浮窗的交互。
- 扩大额度详情的悬浮交互范围，保留订阅服务返回的认证错误信息。
...
- 新增订阅用量监控、历史与周期估算，并提供 macOS 原生浮窗及可配置的展示偏好。
- 新增任务只读上下文和诊断信息，便于查看 Agent 活动、项目与运行状态。
- 扩展 HTTPS 隧道服务商支持，完善连接配置与应用更新流程。
- 优化主题、配额提示、首页交互和桌面外观。

支持 Apple Silicon Mac 与 Windows x64。macOS 原生浮窗仅在 macOS 提供。

## 0.7.3

OAuth 认证、桌面体验与发布构建优化。

- HTTPS 连接新增 OAuth 认证，支持本机授权确认、PKCE、客户端注册、令牌刷新与撤销，并提供授权管理界面。
- 完善 Agent 适配、发现与排序，以及连接配置和设置面板交互。
- 支持主题配色与半透明窗口，改进 Windows 原生标题栏、品牌图标和桌面视觉反馈。
- 发布流程并行执行双端检查与构建，复用已验证产物；优化 Rust 发布编译并行度，缩短原生重编耗时。
