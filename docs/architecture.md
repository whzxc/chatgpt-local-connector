...
连接方式分为官方 Tunnel 与自备 HTTPS MCP。官方 Tunnel Client 独立下载管理；HTTPS 模式由 Rust 提供独立的 Streamable HTTP 监听，仅暴露 MCP、支持 Bearer 访问密钥或无需认证，TLS 与公网转发由用户自备代理负责。两种入口复用相同工具分发、审批和回执，不能同时开启。默认由 Codex Desktop 拥有任务执行权，Connector 通过 Desktop IPC 管理任务。关闭「自动打开 Codex 任务」后，新任务由 Connector 持有的 app-server 后台执行，不唤起 Desktop。执行归属按任务持久化，切换开关不会迁移已有任务；查询、续接和中断沿用任务原有路径。
