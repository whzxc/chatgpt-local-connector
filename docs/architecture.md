...
41 个工具定义保存在 Rust 编译内嵌的 `native/src/catalog.json` 中；参数由 JSON Schema 校验，写请求有持久化回执。大量输出可分页，事件持久化与界面日志分开，常规界面不展示协议握手噪声。
...
当前通用 Agent 工具尚未暴露 `agent_wait`；Codex 的原生 `codex_wait` 保持可用。
