...
macOS 从 Finder 或 Dock 启动应用时，通常不会继承终端中的代理环境变量。每次开启连接，应用会在未显式设置代理环境变量时读取当前 macOS 系统手动 HTTP/HTTPS 代理，并将其传给 Tunnel 的初始化和运行进程；修改系统代理后需关闭并重新开启连接。连接日志中的代理来源提示不包含地址或认证信息。

如果应用进程已经设置 `HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY` 或对应小写变量（包括显式空值），则保留这套配置，不自动混入系统代理。已有 `NO_PROXY`/`no_proxy` 规则会保留，并补入 `localhost`、`127.0.0.1` 和 `::1`。本机健康检查、stdio 转发及预览请求始终直连。

自动读取仅支持 macOS 的手动 HTTP/HTTPS 代理，不执行 PAC/WPAD，也不导入系统代理例外列表或仅 SOCKS 的配置；这些场景请显式提供代理环境变量及 `NO_PROXY`。Windows 保持继承环境变量的行为。此设置作用于 Tunnel 及其子进程，不更改系统代理，也不配置客户端安装下载的代理。
