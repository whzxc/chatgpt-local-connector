...
test('MCP lists all existing native and generic agent tools and seven discoverable native domains with valid object schemas', async () => {
  ...
  assert.deepEqual(inventory.tools.map(tool => tool.name).sort(), [...existing, 'agents', 'agent_capabilities', 'agent_tasks', 'agent_create', 'agent_read', 'agent_send', 'agent_interrupt', 'agent_events', 'agent_pending', 'agent_respond', 'agent_request', 'fs', 'command', 'process', 'mcp', 'file_search', 'codex_thread', 'codex_account'].sort());
