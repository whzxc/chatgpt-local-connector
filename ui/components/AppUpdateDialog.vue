  ...
  <Teleport to="body">
    <dialog ref="dialog" class="app-update-dialog" aria-labelledby="app-update-title" aria-describedby="app-update-description" @cancel.prevent="dismissUpdate" @close="dismissUpdate">
      <header class="update-heading">
        ...
        <div><h2 id="app-update-title">{{available ? t('versionValueIsAvailable', { version: update?.version }) : t('appUpdates')}}</h2><p id="app-update-description">{{ t('installationRestartsTheAppAndRestoresThePrevious') }}</p></div>
        <button class="ghost dismiss" :aria-label="t('remindMeLater')" :disabled="active" @click="dismissUpdate"><X aria-hidden="true"/></button>
      </header>
      <div class="update-body">
        ...
        <div v-if="active" role="status" aria-live="polite">
          <p>{{phase === 'installing' ? t('installingRestartingSoon') : progress === undefined ? t('downloadingValueMb', { size: (downloaded / 1048576).toFixed(1) }) : t('downloadingValue', { percent: progress })}}</p>
          <progress v-if="phase==='downloading'" :value="progress" max="100" :aria-label="t('updateDownloadProgress')"/>
        </div>
      ...
      <footer>
        <button v-if="available && !active" class="text-button" @click="skip">{{ t('skipThisVersion') }}</button>
        <button v-if="error && !active" class="text-button" @click="openDownloads">{{ t('downloadManually') }}</button>
        <div class="update-actions">
          <button v-if="!active" autofocus @click="dismissUpdate">{{ t('later') }}</button>
          <button v-if="phase==='downloading'" @click="cancel">{{ t('cancelDownload') }}</button>
          <button v-if="available" class="primary" :disabled="active || checking" @click="install">{{phase==='installing' ? t('installing') : phase==='downloading' ? t('downloading') : error ? t('retryInstallation') : t('downloadAndInstall')}}</button>
        </div>
      </footer>
    </dialog>
  </Teleport>
