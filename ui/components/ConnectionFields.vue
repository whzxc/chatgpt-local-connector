...
const props = defineProps<{ form: ConnectionForm; config?: Config; disabled?: boolean; allowTunnel: boolean }>();
...
<template>
  <NFormItem :label="t('connectionMethod')" path="connectionMode"><NInput v-if="!allowTunnel" value="HTTPS MCP" readonly :input-props="{'aria-label':t('connectionMethod')}"/><SingleChoice v-else v-model:value="form.connectionMode" :label="t('connectionMethod')" :disabled="disabled" :options="[{label:'OpenAI Secure Tunnel',value:'tunnel'},{label:'HTTPS MCP',value:'https'}]"/></NFormItem>
  <template v-if="!https">
    <NFormItem path="tunnelId" :rule="required()" :show-require-mark="false" :label-style="{width:'100%',display:'grid',gridTemplateColumns:'minmax(0,1fr)'}">
      <template #label><span class="credential-heading"><span>Tunnel ID *</span><NButton text type="primary" size="tiny" :aria-label="t('getCredentials')" @click="run('tunnel-help',()=>openUrl('https://platform.openai.com/settings/organization/tunnels'))">{{t('getCredentials')}}</NButton></span></template>
    ...
    <NFormItem path="apiKey" :rule="config?.hasApiKey ? undefined : required()" :show-require-mark="false" :label-style="{width:'100%',display:'grid',gridTemplateColumns:'minmax(0,1fr)'}">
      <template #label><span class="credential-heading"><span>Runtime API Key</span><NButton text type="primary" size="tiny" :aria-label="t('getKey')" @click="run('api-key-help',()=>openUrl('https://platform.openai.com/settings/organization/api-keys'))">{{t('getKey')}}</NButton></span></template>
  ...
  <template v-else>
    <NFormItem :label="t('provider')" path="httpsProvider"><SingleChoice v-model:value="form.httpsProvider" :label="t('provider')" :disabled="disabled" :options="[{label:'Cloudflare',value:'cloudflare'},{label:'ngrok',value:'ngrok'},{label:t('customDomain'),value:'custom'}]"/></NFormItem>
...
<style scoped>.listener-options{margin:0 0 24px}.credential-heading{display:flex;width:100%;align-items:center;justify-content:space-between;gap:16px}</style>
