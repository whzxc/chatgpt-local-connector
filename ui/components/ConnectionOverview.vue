...
const IngressDetailsDialog = defineAsyncComponent(() => import('./IngressDetailsDialog.vue'));
const IngressDialog = defineAsyncComponent(() => import('./IngressDialog.vue'));
const AgentsSettings = defineAsyncComponent(() => import('./AgentsSettings.vue'));
const agentsDialog = ref(false);
const agentOrigin = ref({x:0,y:0,size:30});
const emit = defineEmits<{ panel: [open:boolean] }>();
function showAgents() {
  ...
  agentsDialog.value=true; emit('panel',true);
}
function closeAgents() {
  agentsDialog.value=false; emit('panel',false);
...
const agents = computed(() => inventory.agents.value.filter(a => a.installed && a.available !== false).sort((a,b) => Number(b.agent==='codex')-Number(a.agent==='codex') || name(a).localeCompare(name(b))).slice(0,4));
...
function path(index: number, count: number, right = false) {
  ...
  <section class="connection-core" :aria-label="t('connectionStatus')">
    ...
    <div class="connection-graph" :style="{height:`${height}px`}">
      <div class="graph-nodes sources">
        <NTooltip v-if="!sources.length" :show="nodeTip==='default-source'" placement="top"><template #trigger><button class="graph-node" aria-label="ChatGPT" @mouseenter="nodeTip='default-source'" @mouseleave="nodeTip=''" @focus="nodeTip='default-source'" @blur="nodeTip=''" @keydown.esc="nodeTip=''" @click="openSource()"><SourceIcon platform="chatgpt"/></button></template>ChatGPT</NTooltip>
...
.wire-port.live{border-color:light-dark(#3b7860,#9cc6ab)}
