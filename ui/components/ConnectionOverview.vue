...
import { Ellipsis, Plus, Power, LoaderCircle, CircleCheck, CircleDashed, CircleAlert, CirclePause } from '@lucide/vue';
import { t } from '../i18n';
...
function openSource(source?: Ingress) {
  nodeTip.value = '';
  source ? view(source) : edit(undefined, true);
}
...
function openAgent(agent: Agent, event?: MouseEvent) {
  ...
  if(rect) detailOrigin.value={x:rect.x,y:rect.y,size:rect.width};
...
function markerParameter(curve: ReturnType<typeof wire>, right: boolean) {
  const rightOrigin = wireWidth(false) + (compactGraph.value ? 124 : 190);
...
<style scoped>
.graph-brain .graph-github{position:absolute;z-index:4;left:calc(50% + var(--ring-offset-x));top:calc(50% + var(--graph-ring-radius) + var(--ring-offset-y));box-sizing:border-box;width:40px;height:40px;border:var(--graph-line-width) solid var(--accent);border-radius:50%;background:var(--graph-bg);color:var(--accent);opacity:0;pointer-events:none;transform:translate(-50%,-50%) scale(0);transform-origin:center;transition:opacity 120ms ease,transform 320ms cubic-bezier(.2,.8,.2,1.15)}
.graph-brain:hover .graph-github,.graph-brain:focus-within .graph-github{opacity:1;pointer-events:auto;transform:translate(-50%,-50%) scale(1)}
