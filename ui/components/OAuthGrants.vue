...
import { NAlert, NButton, NCard, NEmpty, NSpin, NTooltip } from 'naive-ui';
...
interface Pending { id: string; clientName: string; redirectUri: string; status: 'pending' | 'waiting' | 'expired'; }
interface Grant { id: string; clientName: string; }
const grants = ref<Grant[]>([]), pending = ref<Pending[]>([]);
...
async function refresh(afterAction = false) {
  ...
  if (!props.running) { grants.value = []; pending.value = []; error.value = ''; return; }
...
async function approve(request: Pending) {
  ...
  if (!await action('decision', { id: request.id, allow: true })) request.status = 'pending';
}
...
<template>
  <section class="oauth-grants">
    ...
    <NCard v-for="request in pending" :key="request.id" class="oauth-request" size="small">
      ...
      <NButton v-else type="primary" :disabled="busy || disabled || !running" @click="approve(request)">{{t('oauthAllow')}}</NButton>
      <p v-if="request.status === 'waiting'" class="oauth-caption">{{t('oauthWaiting')}}</p>
      </div>
    </NCard>
...
.oauth-decision{display:flex;flex-direction:column;align-items:center;padding:14px 0 18px;overflow-x:auto}.oauth-caption{margin:12px 0 0;color:var(--muted);font-size:14px;text-align:center}.oauth-request{margin-bottom:12px}.oauth-request-title{display:flex;align-items:baseline;flex-wrap:wrap;gap:4px 8px;padding-right:12px;font-size:14px;overflow-wrap:anywhere}.oauth-request-title small{font-size:12px;font-weight:400;color:var(--muted)}
</style>
