...
const overrides = computed<GlobalThemeOverrides>(() => ({
  common: { boxShadow1: 'var(--shadow-popover)', boxShadow2: 'var(--shadow-panel)', boxShadow3: 'var(--shadow-surface)', primaryColorSuppl: dark.value ? accent.value.dark : accent.value.light, primaryColor: dark.value ? accent.value.dark : accent.value.light, primaryColorHover: tint(dark.value ? accent.value.dark : accent.value.light, 255, .15), primaryColorPressed: tint(dark.value ? accent.value.dark : accent.value.light, 0, .15), borderRadius: '8px', fontFamily: "Inter,-apple-system,BlinkMacSystemFont,'Segoe UI','PingFang SC',sans-serif", fontSize: '13px', heightMedium: '36px' },
