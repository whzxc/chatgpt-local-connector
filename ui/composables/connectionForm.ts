import { configDefaults } from '../ingressConfig';
...
export function connectionForm(config?: Config): ConnectionForm {
  ...
  return { domain, connectionMode: config?.connectionMode || 'tunnel', httpsProvider: config?.httpsProvider || 'ngrok', cloudflareMode: config?.cloudflareMode || 'quick', httpsHost: config?.httpsHost || String(configDefaults.httpsHost), httpsPort: config?.httpsPort || Number(configDefaults.httpsPort), tunnelId: config?.tunnelId || '', apiKey: '', cloudflareToken: '', ngrokAuthtoken: '', pinggyToken: '', localxposeAccessToken: '', pinggyMode: config?.pinggyMode || 'quick', localxposeMode: 'named', localxposeRegion: config?.localxposeRegion || 'us', ngrokMode: config?.ngrokMode || 'named' };
}
