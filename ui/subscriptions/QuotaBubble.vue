...
<template>
  <div class="quota-bubble" :class="{embedded}">
    <UsageHistory :rail="!embedded" :detail-placement="detailPlacement" @popover="emit('popover',$event)" :history="provider.history" :reset-count="provider.availableResetCount" :reset-credits="provider.resetCredits" :now="now" :details="details" v-slot="{register,hover,focus,expanded,displayed}">
      <header>
