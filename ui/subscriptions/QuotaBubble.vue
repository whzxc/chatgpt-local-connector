...
const noticeIcon=ref<HTMLElement>(), noticeHovered=ref(false);
...
onMounted(async()=>{
  ...
  const stop=await listen<{x:number;y:number}>('usage-panel:hover',({payload:{x,y}})=>{
    const r=noticeIcon.value?.getBoundingClientRect();
    noticeHovered.value=!!r&&x>=r.left&&x<=r.right&&y>=r.top&&y<=r.bottom;
  });
