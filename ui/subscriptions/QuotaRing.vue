...
<template><span class="quota-ring" :aria-busy="running" :class="{stale:provider.state==='stale',unknown:left===undefined}" :style="{'--quota-color':quotaColor(current,provider,warningAt)}"><span class="quota-dial"><svg viewBox="0 0 36 36" aria-hidden="true"><circle cx="18" cy="18" r="18" class="track"/><circle v-if="left!==undefined" cx="18" cy="18" r="18" class="value" :stroke-dasharray="`${Math.max(0,Math.min(100,progress.value.value[0]!))/100*113.097} 113.097`"/></svg><ActivityArc v-if="running"/><img :src="brandIcon(provider.agentId)" alt=""/></span><span v-if="showPercentage!==false" class="quota-number" :aria-label="`${quotaLabel()} ${quotaPercent(current)}`">{{quotaPercent(current)}}</span></span></template>
<style scoped>
.quota-ring{display:flex;align-items:center;flex-direction:column;gap:6px;width:40px;color:#f5f5f7;font-variant-numeric:tabular-nums}.quota-dial{width:36px;height:36px;position:relative}.quota-dial>svg:not(.activity-arc){width:36px;height:36px;overflow:visible;transform:rotate(-90deg)}circle{fill:none;stroke-width:4}.track{stroke:#303033}.value{stroke:var(--quota-color);stroke-linecap:round}.quota-dial img{position:absolute;inset:10px;width:16px;height:16px;filter:brightness(0) invert(1)}.quota-number{font-family:ui-rounded,'SF Pro Rounded',-apple-system,sans-serif;font-size:13px;font-weight:500;line-height:16px}.stale{opacity:.55}.unknown .quota-number{color:#88888c}
</style>
