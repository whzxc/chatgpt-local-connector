...
const props=defineProps<{rail?:boolean;valueTrigger?:boolean;detailPlacement?:'left'|'right';dismissKey?:number;history?:UsageHistory;resetCount?:number|null;resetCredits?:ProviderSnapshot['resetCredits'];now:number;details?:{id:string;lines:string[]}[]}>();
...
watch(()=>props.dismissKey,()=>{clearTimeout(hoverTimer);hoverTarget='';expanded.value=false;});
...
<style scoped>
.usage-history{display:flex;flex-direction:column;font-size:12px;line-height:1.4}
...
.usage-history>.history-period{padding:4.5px 0;box-sizing:border-box}
.usage-history>.history-period:first-child{padding-top:0}
.usage-history>.history-period:last-child{padding-bottom:0}
