...
onMounted(async()=>{document.addEventListener('keydown',keydown);if(isDesktop){const {listen}=await import('@tauri-apps/api/event');const stop=await listen<{side:'left'|'right'}>('tray-panel:open',e=>{side.value=e.payload.side;void opened();});if(disposed)stop();else off=stop;}await readStatus().catch(e=>error.value=String(e));});
onUnmounted(()=>{clearTimeout(copyTimer);disposed=true;off?.();document.removeEventListener('keydown',keydown);});
</script>
<template>
  <div class="tray-surface" @mousedown.self="dismiss">
    <section class="tray-card" :class="side" :aria-label="t('trayUsage')">
      ...
      <footer><span class="app-version">Local Connector {{version}}</span><NButton text size="small" class="connection-menu" :loading="busy" :disabled="!isDesktop" :aria-label="`${t(connection?'trayConnected':'trayDisconnected')} · ${t('trayOptions')}`" aria-haspopup="menu" @click="openOptions"><span>{{t(connection?'trayConnected':'trayDisconnected')}}</span><ChevronRight :size="12" aria-hidden="true"/></NButton></footer>
    </section>
  </div>
</template>
<style scoped>
.tray-surface{position:fixed;inset:0;color:var(--ink)}
.tray-card{position:absolute;top:0;bottom:0;width:380px;max-width:100%;box-sizing:border-box;display:flex;flex-direction:column;background:var(--surface);border:1px solid var(--line);border-radius:22px;overflow:hidden}.tray-card.left{right:0}.tray-card.right{left:0}
.tray-scroll{min-height:0;overflow:auto;padding:14px;display:flex;flex-direction:column;gap:14px;scrollbar-width:thin}.provider-card{background:var(--soft);border-radius:18px;padding:14px;flex:none}footer{padding:8px 14px;min-height:22px;gap:12px;display:flex;justify-content:space-between;align-items:center;border-top:1px solid var(--line);flex:none;font-size:11px;color:var(--muted)}.app-version{white-space:nowrap}.connection-menu{flex:none;font-size:11px;color:var(--muted);gap:4px}
</style>
